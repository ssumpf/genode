...
struct Drm::Buffer
{
	...
	bool mmap(Genode::Env &env)
	{
		...
		_local_addr = addr_t(env.rm().attach(_allocation.cap, _allocation.size,
		                                     _allocation.offset));
		return true;
	}
	...
	addr_t mmap_addr() const { return _local_addr; }
...
struct Drm::Context
{
	...
	void _map_buffer_gpu(Buffer &buffer, Gpu_virtual_address vaddr)
	{
			...
			try {
				_buffer_space.apply<Buffer>(id, [&](Buffer &b) {
					...
					if (!b.gpu_vaddr_valid)
						_map_buffer_gpu(b, Gpu_virtual_address { .addr = obj[i].offset });
