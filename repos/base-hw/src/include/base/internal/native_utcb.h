...
class Genode::Native_utcb
{
	...
	private:
		...
		/*
		 * Note thats the member variables are put in to a header structure, this
		 * way the size of this header structure is padded by the compiler to a
		 * machine word size boundary and '_data' is in turn aligned to machine word
		 * boundary. This also makes the dimensioning of '_data' easy (page size -
		 * size of  header).
		 */
		struct {
