...
/* enable MIPI power domain */
	...
	/*
	 * multiply by 60
	 * output MHz = input Clock/input divider * Pll_video_1_1 * 8 / output divider
	 * output 120MHz = 25/5 * 60 * 8 / 2
	 *
	 * All dividers are values are n + 1, but output divider which is (n+1)*2
	 */
	...
	/* MIPI_DSI_ESC_RX_CLK_ROOT = SYSTEM_PLL1_DIV10 80 MHz*/
	ccm.write<Ccm_reg::Target_root_36>(0x12000000);
	ccm.write<Ccm_reg::Target_root_36::Prediv>(0);

	/* set pixel reference clock to VIDEO_PLL1_CLOCK 120 MHz */
