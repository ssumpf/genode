...
{
	//log(__func__, ": procname='", (char const *)procname, "'", " egl: ", (void *)eglGetProcAddress((char const*)procname));
...
{
	Genode::warning(__func__, " called from ", __builtin_return_address(0));
                            ...
                            GLXContext shareList, Bool direct)
{
	...
	Genode::warning(__func__, " ctx: ", ctx,
...
{
	//Genode::warning(__func__, " called from ", __builtin_return_address(0));
	//Genode::log("display: ", display, " drawable: ", drawable, " context: ", ctx);
