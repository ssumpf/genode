...
class Igd::Mmio : public Genode::Mmio
{
	...
	public:
		                      ...
		                      uint32_t pitch, bool x)
		{
			...
			typename FENCE_REG::access_t const l = lower >> 12; //FENCE_REG::Fence_lower_bound::get(lower);
			typename FENCE_REG::access_t const u = upper >> 12; //FENCE_REG::Fence_upper_bound::get(upper);
			...
			// FIX
			// write<FENCE_REG>(id, v);
			// asm volatile("mfence":::"memory");
			// (void)read<FENCE_REG>(id);

			volatile uint64_t *reg = (uint64_t*)(Mmio::base() + 0x100000 + (id * sizeof(uint64_t)));
			*reg = v;
			asm volatile("mfence":::"memory");
			(void)*reg;
