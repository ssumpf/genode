...
class Igd::Ppgtt_context : public Igd::Common_context_regs
{
	...
	public:
			...
			Common_context_regs(base)
		{
			...
			{
				typename Pdp_0_udw_value::access_t v = read<Pdp_0_udw_value>();
				Genode::uint32_t const udw = (Genode::uint32_t)((plm4_addr >> 16) >> 16);
				Pdp_0_udw_value::Value::set(v, udw);
				write<Pdp_0_udw_value>(v);
			}
			write_offset<Pdp_0_ldw_mmio>(RING_BASE);
			{
				typename Pdp_0_ldw_value::access_t v = read<Pdp_0_ldw_value>();
				Genode::uint32_t const ldw = (Genode::uint32_t)plm4_addr;
				Pdp_0_ldw_value::Value::set(v, ldw);
				write<Pdp_0_ldw_value>(v);
			}
...
class Igd::Bcs_context
{
	public:

		enum {
			CONTEXT_PAGES = 2 /* ctx */ + 1 /* GuC */,
