...
class Main
{
	private:
		...
		Platform::Device_client  _fw_dev  {
			_platform.device_by_property("compatible", "qemu,fw-cfg-mmio") };
		Attached_dataspace       _fw_mem { _env.rm(),
			_fw_dev.io_mem_dataspace() };
		Fw                       _fw { (addr_t)_fw_mem.local_addr<addr_t>() };

		Ram_dataspace_capability _fb_ds_cap  {
			_platform.alloc_dma_buffer(SCR_HEIGHT * SCR_STRIDE) };
		Attached_dataspace       _fb_ds      { _env.rm(), _fb_ds_cap };

		Ram_dataspace_capability _config_ds_cap  {
			_platform.alloc_dma_buffer(0x1000) };
		...
		void _setup_framebuffer(Fw_config_file const &file)
		{
			...
			addr_t config_phys = (addr_t)_platform.bus_addr_dma_buffer(_config_ds_cap);
			addr_t fb_phys     = (addr_t)_platform.bus_addr_dma_buffer(_fb_ds_cap);
