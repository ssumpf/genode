...
void lx_emul_acpi_init()
{
	...
	env().devices.for_each([&] (Device &d) {
		...
		auto add_iomem = [&] (unsigned long  base, unsigned long  size) {
			device.resources.add(LX_EMUL_ACPI_IOMEM, base, size);
		};

		auto add_ioport = [&] (unsigned short base, unsigned short size) {
			device.resources.add(LX_EMUL_ACPI_IOPORT, base, size);
		};

		auto add_irq = [&] (unsigned number) {
			device.resources.add(LX_EMUL_ACPI_IRQ, number, 1);
		};

		d.for_each_resource(add_iomem, add_ioport, add_irq);
	});
