...
int acpi_bus_get_device(acpi_handle handle, struct acpi_device **device)
{
	...
	*device = lx_emul_acpi_device(handle);
	if (!*device)
		return -ENODEV;
...
{
	static unsigned long virt_handle = 0x1000;
	struct acpi_device *adev =
		acpi_add_single_object((acpi_handle)virt_handle++, dev_name(&dev->dev));
